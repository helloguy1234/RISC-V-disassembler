...
- [ ] 1 **[Step 1] IR & Semantic Lifting:** Nâng cấp cấu trúc `InstructionIr` thành hệ thống IR ngữ nghĩa bằng cách đính kèm cây AST (Abstract Syntax Tree) cho từng lệnh. Bước này chỉ mô hình hóa MỘT lệnh đơn lẻ, không liên kết/thế biến với các lệnh khác.
...
- [ ] 1 **[Step 2] Abstract Domain:** Xây dựng Miền trừu tượng cấu trúc (Abstract Domain D) để ghép nối các AST đơn lẻ ở Step 1 thành một phương trình toán học lớn (`TA = Base + Index * Scale`).
...
- [ ] 1 **[Step 3] Jump Table Analyzer:** Xây dựng module xử lý `jalr` bằng lý thuyết SJA (Structural Tracking, Bounds Analysis - Lan truyền ràng buộc, Fixpoint Iteration).
...
## Kế hoạch Thiết kế Chi tiết (Detailed Design Plan)

### 1. Hệ thống AST cho Step 1 (`org.hello.riscvdisassembler.core.decode.model.ast`)

Mục tiêu: Định nghĩa cấu trúc dữ liệu bất biến (immutable) để lưu trữ phương trình toán học của MỘT lệnh đơn lẻ. Khai báo bằng tính năng `sealed interface` và `record` của Java 21 để hỗ trợ Pattern Matching sau này.

- **`Expression`** _(sealed interface)_: Giao diện gốc đại diện cho mọi biểu thức tính toán được.
- **`RegisterExpr`** _(record)_: Nút lá. Chứa `String name` (VD: "x5", "a0").
- **`ImmediateExpr`** _(record)_: Nút lá. Chứa `long value` (hằng số).
- **`UnknownExpr`** _(record)_: Nút lá. Đại diện cho giá trị không xác định ($\top$).
- **`Operator`** _(enum)_: Các toán tử đại số cơ bản: `ADD`, `SUB`, `MUL`, `SHIFT_LEFT`, `SHIFT_RIGHT`, `AND`, `OR`, `XOR`.
- **`BinaryOpExpr`** _(record)_: Nút cành đại diện phép tính 2 ngôi. Chứa `Operator op`, `Expression left`, `Expression right`.
- **`MemoryLoadExpr`** _(record)_: Nút cành đại diện phép đọc bộ nhớ (như lệnh `lw`). Chứa `Expression baseAddress` và `int sizeBytes`.
- **`AssignExpr`** _(record)_: Bao bọc lại toàn bộ lệnh. Chứa `RegisterExpr lhs` (thanh ghi đích) và `Expression rhs` (cây biểu thức vế phải).

_Tích hợp:_

- Sửa đổi record `InstructionIr`: Thêm trường `AssignExpr semantic`.
- Sửa đổi `Rv32iDecoder`: Cần lập kế hoạch ánh xạ từ `Mnemonic` sang `Operator` và khởi tạo `AssignExpr` ngay trong quá trình giải mã byte.
